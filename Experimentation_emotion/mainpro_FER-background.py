...
path = os.path.join(opt.dataset + '_' + opt.model + 'neutral_random')
...
#             print('Batch_idx: %d | Loss: %.3f | Acc: %.3f%% (%d/%d)' % (batch_idx, train_loss/(batch_idx+1), 100.*correct/total, correct, total))
...
def PublicTest(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(PublicTestloader):
        ...
        if batch_idx == len(PublicTestloader)-1:
...
def PrivateTest(epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(PrivateTestloader):
        ...
        if batch_idx == len(PrivateTestloader)-1:
            utils.progress_bar(batch_idx, len(PublicTestloader), 'Loss: %.3f | Acc: %.3f%% (%d/%d)'
